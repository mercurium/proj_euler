...
for i in xrange(1,SIZE + 1):  #For each square from 1 to 100
    #number of primes used, going in reverse to avoid compounding adds.
    for j in xrange(min(i-1,NUM_PRIMES-1),-1,-1): 
        #We don't need to check all indices, only up to a certain SIZE. Either up to the end 
        #of the array if we have large numbers, or until what could have occured already.
