...
def mod_count(n,mod):
  return sum([ n/mod**x for x in xrange(1,20)])
...
for i in xrange(len(prime_nums)):
  if mod_count(BIG,prime_nums[i]) - mod_count(SMALL,prime_nums[i]) - mod_count(BIG-SMALL,prime_nums[i]) > 0:
    ...
    numerator = reduce(mult, [factorial(BIG/prime_nums[i]**power,prime_nums[i]) for power in xrange(0,20)], 1)
    denom1    = reduce(mult, [inverted_fa(SMALL/prime_nums[i]**power,prime_nums[i]) for power in xrange(0,20)],1)
    denom2    = reduce(mult, [inverted_fa((BIG-SMALL)/prime_nums[i]**power,prime_nums[i]) for power in xrange(0,20)],1)
    mod_vals[i] = numerator * denom1 * denom2
